 ...
 */
function binlMD5(x: Array<number>, len: number): Array<number> {
  /* append padding */
  x[len >> 5] |= 0x80 << len % 32;
  x[(((len + 64) >>> 9) << 4) + 14] = len;
...
function rstrHMACMD5(key: string, data: string) {
  ...
  var ipad = new Array<number>();
  var opad = [];
  var hash: Array<number>;
  ipad[15] = opad[15] = 0;
  if (bkey.length > 16) {
...
export default function md5(value: string): string {
  return rawHMACMD5('', value);
}

export function md5Key(value: string, key: string): string {
  if (!key) {
    return rawMD5(value);
  }
...
export function md5KeyRaw(value: string, key: string, raw: string): string {
  if (!key) {
    if (!raw) {
      return hexMD5(value);
    }
